        ...
        <li v-for="topic in topices" :key="topic.id">
          <el-card class="box-card" shadow="never">
            <div class="text item">
              ...
              <div id="context">{{ contextlimit(topic.context) }}</div>
              <div>
                <Picture
                  v-if="topic.picture"
...
import { getCollectTopic, getCollectTopicCount } from "../../network/collect";
export default {
  name: "CollectTopic",
  components: {
    Picture
  ...
  methods: {
    getTopic() {
      getCollectTopic(this.$store.state.id, this.index)
        .then(response => {
    ...
    getcount() {
      getCollectTopicCount(this.$store.state.id)
        .then(res => {
